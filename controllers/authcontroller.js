...
// ----------------------------------------------------
// GMAIL SMTP SETUP 🔥 (No Blocks, Fast Delivery)
// ----------------------------------------------------
const transporter = nodemailer.createTransport({
  host: 'smtp.gmail.com',
  port: 465,
  secure: true, 
  auth: {
    user: process.env.GMAIL_EMAIL,        // Aapka Gmail ID (.env se aayega)
    pass: process.env.GMAIL_APP_PASSWORD, // Aapka 16-digit App Password (.env se aayega)
  },
});
...
const forgotPassword = async (req, res) => {
  try {
    ...
    // Email ka format aur details
    const mailOptions = {
      from: `"AuthApp Support" <${process.env.GMAIL_EMAIL}>`, // Sender email dynamic kar di
      to: user.email,
      subject: 'Password Reset Request',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
    ...
    // Cleanup agar mail na jaye (Safety fix added '?')
    if (req?.body?.email) {
      const user = await User.findOne({ email: req.body.email });
      if (user) {
        user.resetPasswordToken = undefined;
